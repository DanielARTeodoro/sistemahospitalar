 ...
 * Sample Skeleton for 'endermidad.fxml' Controller Class
 */
...
public class EnfermidadeController {
	...
	@FXML // fx:id="txtEnfermidade0"
	private TextField txtEnfermidade0; // Value injected by FXMLLoader
	...
	void handleSubmitButtonAction(ActionEvent event) throws IOException {
		...
		if (txtEnfermidade0.getText().equals("")) {
			Stage stage = (Stage) btnEnfermidade.getScene().getWindow();
			...
			Enfermidade enfe = new Enfermidade();
			enfe.setNome(txtEnfermidade1.getText());
			enfe.setTipo(txtEnfermidade2.getText());
			enfe.setDescricao(txtEnfermidade3.getText());

			
			
			EnfermidadeDAO efdao = new EnfermidadeDAO();
			efdao.inserir(enfe);
	...
	void initialize() {
		assert txtEnfermidade0 != null : "fx:id=\"txtEnfermidade0\" was not injected: check your FXML file 'Enfermidade.fxml'.";
